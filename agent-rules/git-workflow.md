...
4. **Code Reviews & CI/CD**:
   ...
   - **Local Testing Mandatory**: Before pushing any feature branch, the agent **must** run the project verification checks locally to ensure the remote pipeline remains green.
     - **Execution**: Run `./scripts/verify.sh` (bash/unix/Git Bash) or `.\scripts\verify.ps1` (Windows PowerShell).
     - **Git Hooks**: Ensure local git hooks are active by running: `git config core.hooksPath .githooks`. This executes verification automatically before any push.
