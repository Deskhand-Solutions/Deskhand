...
- Reuse `ai_core` services; do not duplicate LLM/embedding logic. For dynamic tool-calling (agent loop), use `LLMRouter.generate_with_tools(organization=..., module_slug=..., prompt=...)` which compiles and runs active integration tools internally.
